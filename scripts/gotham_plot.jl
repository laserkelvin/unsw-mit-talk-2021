...
# ╔═╡ befe5e2a-1656-11ec-09b7-67b31698b347
begin
	using CSV, DataFrames, CairoMakie, Colors
end

# ╔═╡ 4becc156-f89b-487a-8561-990e429e44de
data = CSV.read("DR2_smoothed.txt", DataFrame, delim="\t", header=["Frequency", "Intensity"])

# ╔═╡ 98dcceaf-2924-43ee-9bbd-452fc32f2162
begin
	fig = Figure(resolution=(1000, 400), font="Lato")
	
	ax = Axis(fig[1,1], xlabel="Frequency / MHz", ylabel="T_B / K")
	lines!(ax, data.Frequency, data.Intensity)
	hidespines!(ax, :t, :r)
end

# ╔═╡ ddff93ad-8467-4b0f-960f-b282bba9c125
fig

# ╔═╡ 11ce1231-2c24-4822-806d-01607d164c41
save("gotham-spectrum.png", fig)

# ╔═╡ 086de088-f4b0-4325-a04b-76d9e3579920

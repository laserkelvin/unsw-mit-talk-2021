...
begin
	hight_df = CSV.read("300K-prolate.csv", DataFrame);
	lowt_df = CSV.read("4K-prolate.csv", DataFrame);
	big_df = CSV.read("4K-big.csv", DataFrame);
end

# ╔═╡ c5df9293-f3dd-4137-8bc7-36a2537f568b
names(hight_df)

# ╔═╡ dd05d482-c288-41ae-9637-2ca21aeb49ae
begin
	selected_high = @where(hight_df, hight_df.Position .<= 1e6);
end

# ╔═╡ 4e63ffa3-4c2b-404c-8fda-271ff98e7edb
begin
	fig = Figure(font="Lato", resolution=(1000, 1000), fontsize=18)
	ax1 = Axis(fig[1,1])
	
	ht_plot = barplot!(selected_high.Position, selected_high.Intensity, strokewidth=1, strokecolor=:red, color=:red)

	ax2 = Axis(fig[2,1], xlabel="Frequency / MHz")
	# linkyaxes!(ax1, ax2)
	linkxaxes!(ax1, ax2)
	lowt_plot = barplot!(lowt_df.Position, lowt_df.Intensity, strokewidth=1, strokecolor=:blue)
	Legend(fig[2,2], [lowt_plot], ["4 K"])
	Legend(fig[1,2], [ht_plot], ["300 K"])
...
# ╔═╡ 44c394d4-68fa-4953-81fe-155428b227eb
fig

# ╔═╡ e85d95e1-4e24-48f3-88aa-666332d25336
save("temperature-spectra.png", fig)

# ╔═╡ 8f4fc7be-fb52-4993-adc3-ffc50b29f5dc
begin
	bn_fig = Figure(font="Lato", resolution=(1000, 400), fontsize=24)
	bn_ax = Axis(bn_fig[1,1], xlabel="Frequency / MHz", ylabel="Absolute intensity")
	
	bn_plot = barplot!(lowt_df.Position, lowt_df.Intensity, strokewidth=1, strokecolor=:blue)
	hidespines!(bn_ax, :t, :r)
	xlims!(bn_ax, 0, 1e5)
	bn_ax.xticks = [0, 2e4, 4e4, 6e4, 8e4]
end

# ╔═╡ b9d63f6c-6e2c-4565-addf-626c69919eb1
bn_fig

# ╔═╡ 87048f83-145d-48a7-99c8-67da60ea124d
save("bn_sim_4K.png", bn_fig)

# ╔═╡ ca4cfada-8a32-4723-81df-17bf1d943a6c
begin
	cnn_fig = Figure(font="Lato", resolution=(1000, 400), fontsize=24)
	cnn_ax = Axis(cnn_fig[1,1], xlabel="Frequency / MHz", ylabel="Absolute intensity")
	
	cnn_plot = barplot!(big_df.Position, big_df.Intensity, strokewidth=1, strokecolor=:blue)
	hidespines!(cnn_ax, :t, :r)
	xlims!(cnn_ax, 0, 1e5)
	cnn_ax.xticks = [0, 2e4, 4e4, 6e4, 8e4]
end

# ╔═╡ e969cc3d-5898-4dd1-bd97-ffbe40afea8a
cnn_fig

# ╔═╡ 09d20681-4635-4a28-9542-93dd6633dd25
save("cnn_sim_4K.png", cnn_fig)

# ╔═╡ a0f29ccd-7b23-433b-a6b4-97d7bdd2bfa0
begin
	struct Gaussian{T}
		μ::T
		σ::T
...
begin
	simulate(g::Gaussian, x) = g.A * exp(-(x - g.μ)^2 / (2 * g.σ^2))
	# for a batch of Gaussians
	simulate(g::Vector{Gaussian{T}}, x) where T <: Real = mapreduce(i -> simulate(i, x), +, g)
end
...
git-tree-sha1 = "121f8641845056c31ea478945c08a79fe00ab9c5"
